<?php
/*
Plugin Name: Zengo Custom Thumbnail Image Gallery
Plugin URI: http://www.zengo-web-services.com
Version: 1.0.4
Author: Zengo Web Services
Description: Separate Thumbnail Image from Main Image, Customize Gallery Image Size, Multiupload Images, New images display Styles.
*/
...
/*Add Multiple Start*/
if(isset($_POST['add_multi']) && $_POST['add_multi'] == "UPLOAD")
{
	if(isset($_FILES['files'])){
		...
		foreach($_FILES['files']['tmp_name'] as $key => $tmp_name ){
			...
			$desired_dir = $upload_dir['path'];
			$desired_url = $upload_dir['url'];
			...
			if(empty($errors) == true){
				...
				d_create_thumbnail($file_name);
				
				$title = sanitize_text_field(preg_replace('/[^A-Za-z0-9 \-]/', '', basename($file_name, ".".$ext)));
				$image = $desired_url."/".sanitize_text_field($file_name);
				$timespemp = time() - date('Z');
				$thumbnail = $desired_url."/".sanitize_text_field("thumb_".$timespemp."_".$file_name);
				$success = "";
				$Image_Details = array(
						'title' => $title,
						'thumbnail' => $thumbnail,
		...
		if(empty($error)){
			?>
			<div id="setting-error-settings_updated" class="updated settings-error notice is-dismissible"> 
			<p><strong>MULTIPLE IMAGES ADDED SUCCESSFULLY!</strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button></div>
		<?php
		}else{
			?>
			<div id="setting-error-settings_updated" class="updated settings-error notice is-dismissible"> 
	...
	<script>
		jQuery(document).ready(function(){
			jQuery(".checkbox").click(function(){
				jQuery(".zengo_single").toggle();
				jQuery(".zengo_multi").toggle();
			});
		});
	</script>
        <div class="tabcontents">
            <div id="view1">
				...
				<p>
					<label><input type="checkbox" name="checkbox" class="checkbox" id="checkbox" value="checkbox"> MultiUpload ?</label>
					<div class="zengo_single">
					...
					<div class="zengo_multi" style="display:none;">
						<form action="<?php echo $_SERVER['PHP_SELF']; ?>?page=zengo-gallery&action=add_multi" method="post" enctype="multipart/form-data">
							<table class="zengo-table">
								<tr>
									<th align="left" class="zengo-table">Select Multiple Images</th>
									<td align="center" class="zengo-table">:</td>
									<td class="zengo-table">
										<input type="file" name="files[]" id="main_ImageUpload" multiple="">
									</td>
								</tr>
								<tr>
									<td class="zengo-table" colspan="5">
										<input id="submit_button" type="submit" value="UPLOAD" name="add_multi" class="zengo-save-btn">
									</td>
								</tr>
								<tr>
									<td colspan="4"><strong>NOTE: Use shortcode Generator to display gallery in page.</strong></td>
						 ...
						 <tbody>
							<?php
								...
								<tr>
									...
									<td align="center"><img style="height:100px; max-width:250px;" align="center" src="<?php echo $customthumb[thumbnail]; ?>" ></td>
